 ...
 * `stream`. Without this hint, AI Gate previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // AI Gate accumulates the provider stream and converts it to JSON for the client
  ...
  // AI Gate fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve AI Gate's streaming default unless
