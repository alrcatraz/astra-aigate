...
async function requireToken(apiKeyId?: string): Promise<string> {
  const config = await getObsidianConfigForApiKey(apiKeyId);
  if (!config.token) {
    throw new Error(
...
async function getClient(apiKeyId?: string): Promise<ObsidianClient> {
  const config = await getObsidianConfigForApiKey(apiKeyId);
  const token = await requireToken(apiKeyId);
...
export const obsidianTools = [
  {
    ...
    handler: async (_args: unknown, extra?: McpExtra) => {
      const client = await getClient(extractApiKeyId(extra));
  ...
  },
  {
    ...
    inputSchema: z.object({}),
    handler: async (_args: unknown, extra?: McpExtra) => {
      const token = getSyncToken() || (await requireToken(extractApiKeyId(extra)));
