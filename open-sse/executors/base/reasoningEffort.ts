...
/**
 * Map AI Gate's reasoning-effort inputs onto the binary thinking switch exposed by
...
export function supportsMaxEffortForProvider(provider: string, model: string): boolean {
  ...
  // normalized to xhigh (the AI Gate-internal top tier) and rejected by the
...
// AI Gate carries the requested effort on up to three shapes:
//   1. top-level `reasoning_effort`        — OpenAI / AI Gate-internal
...
export function sanitizeReasoningEffortForProvider(
  ...
  // ONLY as {high, max} (its own top tier is literally "max"). AI Gate's internal
