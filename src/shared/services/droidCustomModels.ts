/**
 * Build the AI Gate `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to AI Gate branding
 * (`custom:AI Gate-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by AI Gate (any index). */
export function isOmniRouteCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:AI Gate");
}
