 ...
 */
export async function reconcileCodexUsageHistory(
  db: DatabaseAdapter,
  input: {
    connectionId: string;
    existing: JsonRecord;
  ...
  const oldIdentity = await resolveUsageAccountIdentity(input.existing);
  const newIdentity = await resolveUsageAccountIdentity(input.merged);
