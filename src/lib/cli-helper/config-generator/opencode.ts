 ...
 * source is the user's OWN AI Gate instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from AI Gate. The catalog is the
...
export async function fetchOmniRouteCatalog(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`AI Gate /v1/models returned ${response.status} ${response.statusText}`);
    }
...
/**
 * Generate a full `opencode.json` document for AI Gate. The catalog is the
...
export async function generateOpencodeConfig(options: GenerateOpencodeOptions): Promise<string> {
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "AI Gate",
