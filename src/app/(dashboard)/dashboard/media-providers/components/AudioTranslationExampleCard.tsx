...
const ENDPOINT_PATH = "/api/v1/audio/translations";
...
function TranslationResultRenderer(data: unknown) {
  if (!data || typeof data !== "object") return null;
  const d = data as Record<string, unknown>;
...
export function AudioTranslationExampleCard({ providerId }: Props) {
  const t = useTranslations("miniPlayground");
  const { apiKey } = useApiKey();
  const { models } = useProviderModels(providerId);

  // Show only audio-translation-capable models. Same narrowing as the STT card:
  // keep entries whose subtype signals translation when present.
  const translationModels = models.filter(
    (m) => (m.type === "audio" && m.subtype === "translation") || (m.type === "audio" && !m.subtype)
  );

  const firstModel = translationModels[0]?.id ?? "whisper-1";
  ...
  const translateQualify = (id: string) =>
    id.startsWith(`${providerId}/`) ? id : `${providerId}/${id}`;
  ...
  const curlSnippet = buildCurl({
    ...
    body: {
      model: translateQualify(effectiveModel),
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      formData.append("model", translateQualify(effectiveModel));
  ...
  const modelOptions = translationModels.length > 0 ? translationModels : [{ id: "whisper-1" }];

  return (
    <PlaygroundCard
      kindLabel={t("audioTranslation")}
      ...
      result={result}
      error={error}
      resultRenderer={TranslationResultRenderer}
    >
      {/* Model */}
      <div>
