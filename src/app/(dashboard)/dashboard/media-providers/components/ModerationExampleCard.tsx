...
const ENDPOINT_PATH = "/api/v1/moderations";
...
export function ModerationExampleCard({ providerId }: Props) {
  ...
  const [input, setInput] = useState<string>("I am going to hurt myself. Please help.");
  ...
  const buildBody = () => ({ model: effectiveModel || undefined, input });
  ...
  const modelOptions = models.length > 0 ? models : [{ id: "omni-moderation-latest" }];

  return (
    <PlaygroundCard
      kindLabel={t("moderation")}
