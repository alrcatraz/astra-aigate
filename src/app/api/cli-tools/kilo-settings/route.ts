...
// Check if AI Gate OpenAI-compatible provider is configured
...
// POST - Configure Kilo Code to use AI Gate as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update AI Gate as openai-compatible provider
...
// DELETE - Remove AI Gate config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "AI Gate settings removed from Kilo Code",
    });
  } catch (error) {
