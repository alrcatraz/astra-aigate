...
// POST — write AI Gate settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge AI Gate settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove AI Gate settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt AI Gate settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
