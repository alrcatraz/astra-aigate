/**
 * MCP Streamable HTTP Transport — /api/mcp/servers/[id]/stream
 *
 * Per-registry-endpoint Streamable HTTP route (gateway mode). Resolves [id]
 * against the mcp_servers registry, then serves the endpoint through the
 * transport layer (builtin tool domains locally, stdio/http via the bridge).
 *
 * Endpoints:
...
async function bridge(request: NextRequest, id: string): Promise<Response> {
  ...
  return handleMcpStreamableHTTP(request, id);
}
