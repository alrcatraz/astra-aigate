 ...
 * `thinkingConfig`, …). The AI Gate router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the AI Gate router,
 ...
 * @param model      resolved AI Gate model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`AI Gate ${upstream.status}: ${errText}`);
      }
